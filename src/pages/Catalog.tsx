          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
            {categories.slice(1).map((cat) => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className="relative h-32 md:h-40 overflow-hidden rounded-sm group text-left"
              >
                <img
                ...
                />
                <div className="absolute inset-0 transition-all" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85) 40%, rgba(0,0,0,0.2))" }} />
                <div className="absolute inset-0 border border-transparent group-hover:border-amber-500/50 transition-colors rounded-sm" />
                <div className="absolute bottom-0 left-0 p-4">
                  <div className="flex items-center gap-2 mb-0.5">
                    {categoryMeta[cat] && <Icon name={categoryMeta[cat].icon as "Volume2"} size={14} className="text-amber-500" />}
                    <span className="font-oswald text-xl font-bold text-white uppercase">{cat}</span>
                  </div>
                  <p className="text-gray-400 text-xs line-clamp-1">{catCounts[cat]} позиций</p>
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="sticky top-20 space-y-4">
              ...
              <div className="glass-card p-4 rounded-sm">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-amber-500/20 rounded-sm pl-9 pr-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-amber-500/50"
                  />
                </div>
              </div>
              ...
              <div className="glass-card rounded-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-amber-500/10">
                  <h3 className="text-xs text-gray-500 uppercase tracking-wider font-medium">Категории</h3>
                </div>
                <ul>
                  {categories.map((cat) => (
                    <li key={cat} className="border-b border-amber-500/5 last:border-0">
                      <button
                        onClick={() => setActiveCategory(cat)}
                      ...
                      >
                        <span className="flex items-center gap-2">
                          {cat !== "Все" && categoryMeta[cat] && (
                            <Icon name={categoryMeta[cat].icon as "Volume2"} size={13} />
                          )}
                          {cat}
                        </span>
                        <span className={`text-xs ${activeCategory === cat ? "text-amber-500" : "text-gray-600"}`}>
                          {cat === "Все" ? equipment.length : catCounts[cat]}
                        </span>
                      </button>
                    </li>
                  ))}
              ...
              {/* Price Filter */}
              <div className="glass-card p-4 rounded-sm">
                <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Цена до</h3>
                <div className="text-amber-500 font-oswald text-2xl font-bold mb-3">{priceMax.toLocaleString()} ₽</div>
                <input
                  type="range"
                  min={500}
                  max={10000}
              ...
              <button
                ...
                className="w-full text-xs text-gray-600 hover:text-amber-500 transition-colors py-2 flex items-center gap-2"
              >
